<template>
  <section class="Articles">
      ...
      <div class="Algolia__Search__Box">
        <div class="Algolia-logo">
          <a target="_blank" title="Algolia Website" href="https://www.algolia.com/">
            <svg height="1.25em" viewBox="0 0 391 124" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="Chunk-selector">
      <label for="chunks">Resultados por Página:</label>
      <select type="text" name="chunks" @change="refresh()" v-model="perpage">
        <option>5</option>
        <option>10</option>
        <option>25</option>
        <option>50</option>
    ...
    <article-page v-for="article in chunk" :key="article.id" :article="article"></article-page>

    <pagination ref="articles" for="articles" :records="dataSet.length" :perPage="perpage" :query="query"></pagination>
  </section>
</template>

<script>
  import articles from '@/json/articles.js'
  import articlePage from './_artigos-page'
  import pagination from './pagination'
  import Bus from '@/events/bus.js'
  import { debounce } from 'lodash'

  const algolia = require('algoliasearch')
  const client = algolia('OYPYOD26VH', 'a38544a0ea6ee32b409cb3f33197bcc8')
  const index = client.initIndex('artigos')

  export default {
    beforeDestroy () {
      Bus.$off('vue-pagination::articles', this.setPage)
    },
    beforeMount () {
      this.setPage()
      Bus.$on('vue-pagination::articles', this.setPage)
    },
    components: {
      articlePage,
      pagination
    ...
    computed: {
      currentChunkStart () {
        return (this.page * this.currentPerPage) - this.currentPerPage
      },
      currentChunkEnd () {
        return this.currentChunkStart + this.currentPerPage
      },
      currentPerPage () {
    ...
    methods: {
      setChunk () {
        let r = this.dataSet.slice(this.currentChunkStart, this.currentChunkEnd)
        this.chunk = r
      },
      refresh () {
        this.page = 1
        this.$refs.articles.setPage(1)
        this.setChunk()
      },
      setPage (p) {
        if (p) {
